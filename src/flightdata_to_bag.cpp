...
int main(int argc, char **argv)
{
    ...
    if ( argc != 6 )
    {
    printf("usage: rosrun rospackage rosnode <Full Path + File nameof the log file generated by AitrData.com (e.g., 2016-02-26_21-16-27_Standard.csv)> <Start time of the video (in milliseconds) from the beginning of the log. This is experimental and does not work properly until further notice. No need to use this as this functionality is taken care of by the neyr argument> <Video Info file name (without path) from DJI> <summerTime = 1 or 0> <Images bagged or not 0 or 1> \n");
    return -1;            
    }
    ...
    //summer time variabls
    int summerTime = atoi(argv[4]);
    int imagesBagged = atoi(argv[5]);
    ...
    //[hint: For the second argument look into the log details on the Airdata website. Go to a particular log, click on Media, then click on Map. Match that info with the corresponding CSV to find the right millisecond, e.g., in case of 2016-02-26_21-16-27_Standard.csv the timestamp is 85400]
    ...
    printf("VideoStartMillisecond = %d\n", VideoStartMillisecond);
    ...
    pitch,roll,cam_yaw,cam_pitch))
    {
        ...
        else
        {
            ...
            {        
                ...
                if(Time_milliSecs == VideoStartMillisecond)
                {
                    VideoStartTimeStampSec = rosTimeSec; VideoStartTimeStampNsec = rosTimeNsec; 
                    printf("Assigning VideoStartMillisecond = %d to the VideoStartTimeStampSec = %ld\n", VideoStartMillisecond, VideoStartTimeStampSec);
                }
                ...
                gpsPosition.header.frame_id = "dji_spark_red_gps_receiver";
                
                sparkOrientation.header.seq = count;
                sparkOrientation.header.stamp = messageTime;
                sparkOrientation.header.frame_id = "dji_spark_red_gps_receiver"; 
                ...
                //make the bag here
                bag.write("/dji_spark_red/gpsPose", messageTime, gpsPosition);
                bag.write("/dji_spark_red/Orientation", messageTime, sparkOrientation);
                bag.write("/dji_spark_red/gimbal/Orientation", messageTime,gimbalOrientation);
                ++count;
            }
        }
    }
    ...
    printf("For Image Date time year = %d , month %d, day %d, hour %d , min %d, sec %d, \n",year,month,day,hour,min,sec);
    printf("Start Image Time = %ld  \n",unix_time_base_for_image);
    ...
    while(!feof(imageFileLogHandler))
    {
        ...
        //imageMsgTime = ros::Time(VideoStartTimeStampSec+(int)image_time, 1000000000*(image_time-(int)image_time));
